module NavigationTags
  ...
  tag "get_main_site_name" do |tag|
    page =  nil
    if tag.locals.page.parent != nil
      if tag.locals.page.parent.parent == nil
        page =  tag.locals.page
      else 
        if tag.locals.page.parent.parent.parent == nil
          page =  tag.locals.page.parent
        else
          if tag.locals.page.parent.parent.parent.parent == nil
            page =  tag.locals.page.parent.parent
          end  
        end       
      end  
      page.title  
  ...
  tag "external-nav-current-page" do |tag|
    page =  nil
    if tag.locals.page.parent != nil
      if tag.locals.page.parent.parent == nil
        page =  tag.locals.page
  ...
  tag "jcvp_part_navigation" do |tag|
    page = tag.locals.page
    html = ""
    page.parts.each do |part|
      html += '<div class="contentNavigation-acitve-item-left-corner">&nbsp;</div>'
      html += "<div class=\"contentNavigation-item\">#{part.name}</div>" if part.name != 'body' and part.name != 'extended'    
    end
    html
  end
  
  tag "jcvp_tab_navigation" do |tag|
    current_page = tag.locals.page
    if current_page.parent != nil and current_page.parent.parent!= nil
      if current_page.parent.parent.parent == nil
        page = current_page
       else
    ...
    for child in published_children
     ...
     html += "<a href=\"#{url}\">#{escape_once(child.breadcrumb)}</a></div>"
    end
    return html
  end
